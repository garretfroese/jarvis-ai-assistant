...
def chat():
    ...
    try:
        ...
        strea@app.route('/api/chat', methods=['POST'])
def chat():
    global current_mode  # Move global declaration to the top
    ...
    try:
        ...
        if message.startswith('!'):
            ...
            if mode_command in ['ceo', 'wags', 'legal', 'default']:
                ...
                return jsonify({
                    "response": f"Mode switched to: {current_mode.upper()}",     "conversation_id": conversation_id,
                    "mode": current_mode,
                    "status": "success"
                })
